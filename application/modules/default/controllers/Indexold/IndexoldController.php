...
class Default_Indexold_IndexoldController extends Zend_Controller_Action
{
	/**
    ...
    public function init()
    {
 		...
 		if($installation_file)
 		{
 			...
 			//show publish date to find sections near to this date
//  			$show_publish_date = $this->_getParam('show_publish_date' );
 			...
 			if(count($website_obj)>0)
 			{
 				...
 				*/
 				...
 				$sections_list = $section->personalized_find('wc_section_storage',array(array('website_id','=',$front_ids->website_id),
																 						array('display_menu','=','yes'),
																 						array('approved','=','yes'),
																 						array('publication_status','=','published'),
																 						array('publish_date','<=',$_SESSION['show_publish_date']),
																 						array('expire_date','>=',$_SESSION['show_publish_date'])),'order_number');
 				...
 				if($sections_list)
 				{
 					foreach ($sections_list as $sec)
 					{
 						...
 						$section_parents = array_reverse(GlobalFunctions::getParentsStorage($branch=array(),$sec->id));
 						if($section_parents)
 							foreach($section_parents as $sp){
 							$sections_arr[$sp->id] = array('id'=>$sp->id,
 									'section_parent_id'=>$sp->section_parent_id,
 									'title'=>$sp->title
 							);
 						}
 							
 						$sections_arr[$sec->id] = array('id'=>$sec->id,
 								'section_parent_id'=>$sec->section_parent_id,
 								'title'=>$sec->title
 						);
 							
 						$section_childs = GlobalFunctions::getChildStorage($branch=array(),$sec->id);
 		
 						if($section_childs)
 							foreach($section_childs as $sc){
 							$sections_arr[$sc->id] = array('id'=>$sc->id,
 									'section_parent_id'=>$sc->section_parent_id,
 									'title'=>$sc->title
 							);
 						}
 				...
 				if(count($sections_arr)>0)
 				{
 					...
 					$html_list = GlobalFunctions::buildHtmlSectionMenu($sections_tree,false,'storage');
 				}
 				...
 				if(!$html_list){
 					$this->_helper->flashMessenger->addMessage ( array (
 							'error' => $lang->translate ( 'There are no publications on date selected' )
 					) );
	...
	public function indexAction()
    {
    	...
    	*/    	
    	...
    	$section_arr = array();
    	//section model
    	$section_obj = new Core_Model_SectionStorage();
    	...
    	{    		
    		...
    		$section_data_arr = $section_obj->find('wc_section_storage', array('id'=>$section_id, 'website_id'=>$front_ids->website_id));  
    	}
    	else 
    	{
    		$section_data_arr = $section_obj->find('wc_section_storage', array('homepage'=>'yes', 'website_id'=>$front_ids->website_id,'publish_date','<=',$_SESSION['show_publish_date'],'expire_date','>=',$_SESSION['show_publish_date']));
    	...
    	if($section_id)
    	{
	    	...
	    	if(count($website_printer_option)>0)
	    	{
	    		...
	    		$section_prints = new Core_Model_SectionPrintsStorage();
	
	    		//save data in wc_section_prints    		
	    		$section_prints_obj = $section_prints->getNewRow('wc_section_prints_storage');
	    		
	    		//Check if section_id has data in wc_section_prints table
	    		$section_prints_data = $section_prints->find('wc_section_prints_storage',array('section_id'=>$section_id));
	    		...
	    		// Save data
	    		$saved_section_print = $section_prints->save('wc_section_prints_storage',$section_prints_obj);    	
	    	}
	    	...
	    	if(count($section_data_arr)>0)
	    	{
		    	...
		    	{	    		
		    		...
		    		{
		    			$section_parent = $section_obj->find('wc_section_storage', array('id'=>$section->section_parent_id));
		    		...
		    		//section area
		    		$section_module_area = new Core_Model_SectionModuleAreaStorage();
		    		$area = $section_module_area->find('wc_section_module_area_storage',array('section_id'=>$section_id));
		    		if(count($area)>0)
		    		{
		    		...
		    		$content = new Core_Model_ContentStorage();
		    		...
		    		//articles
		    		$article_arr = array();
		    		$articles_list = $section_obj->find('wc_section_storage', array('section_parent_id'=>$section->id,'article'=>'yes'), array('order_number'=>'ASC'));
		    		...
		    		if(count($modules)>0)
		    		{
		    			foreach ($modules as $mod)
		    			{
		    				...
		    				$contents_mod = $module_obj->rendercontents($section->id,'yes');
	    	...
	    	if($section_id)
	    		$section_arr = $section_obj->personalized_find('wc_section_storage',array(array('website_id','=',$front_ids->website_id), array('id','!=',$section_id), array('article','=','no'), array('approved','=','yes'), array('publication_status','=','published')),'order_number');
	    	...
	    	if(count($section_arr)>0)
	    	{
		    	foreach ($section_arr as $k => $section)
		    	{
		    		...
		    		//section area
		    		$section_module_area = new Core_Model_SectionModuleAreaStorage();
		    		$area = $section_module_area->find('wc_section_module_area_storage',array('section_id'=>$section->id));
    	...
    	$this->view->storage = 'yes';
    	
    	//enable show old publications
    	$this->view->showold = $website_obj[0]->section_storage;    	
    ...
    public function sitemapAction()
    {
    	...
    	*/
    	...
    	$section_arr = $section_obj->personalized_find('wc_section_storage',array(array('website_id','=',$front_ids->website_id), 
																	    			array('article','=','no'), 
																	    			array('approved','=','yes'), 
																	    			array('publication_status','=','published'),
    																				array('publish_date','<=',$_SESSION['show_publish_date']),
    	...
    	$sections_list = $section->personalized_find('wc_section_storage', array(
    			array('website_id','=',$front_ids->website_id),
    			array('publication_status','=','published'),
    			array('type','=','public'), 
    			array('article','=','no'),
    			array('publish_date','<=',$_SESSION['show_publish_date']),
    			array('expire_date','>=',$_SESSION['show_publish_date'])    			
    	));
    	...
    	if($sections_list)
    	{
    		foreach ($sections_list as $sec)
    		{
    			//section module area
    			$section_module_area = new Core_Model_SectionModuleAreaStorage();
    			$area = $section_module_area->find('wc_section_module_area_storage',array('section_id'=>$sec->id));
    			$area_type = "";
    			if(count($area)>0)
    			{
    	...
    	if(count($sections_arr)>0)
    	{
    		...
    		$list = GlobalFunctions::buildHtmlSitemapTree($sections_tree,false,$website_data['sitemap_level'],1,'storage');
