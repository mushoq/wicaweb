...
CREATE  TABLE IF NOT EXISTS `wc_section` (
  ...
  `order_highlight` smallint(6) DEFAULT NULL,
	`archived` ENUM('yes', 'no') DEFAULT 'no',
  PRIMARY KEY (`id`, `website_id`) ,
  UNIQUE INDEX `url` USING BTREE (`url`),
...
CREATE  TABLE IF NOT EXISTS `wc_form_field` (
  ...
  `name` VARCHAR(100) NOT NULL ,
  `description` VARCHAR(250) NULL ,
  `type` ENUM('textfield', 'emailfield', 'datepicker', 'textarea', 'radiobutton', 'dropdown','checkbox','comment','file') NOT NULL ,
