...
//				break;
	...
	*/
	$.fn.agileUploaderPreview = function(file) {
		...
		//this section was modified only for wicaweb project
		//load preview image on preview field
		if($('#single_img') != null)
			$("#prw_image").attr('src', file.base64Thumbnail);
...
//		 Re-direct or empty the list so another submission can be made
		if(typeof(opts.submitRedirect) == 'string') {
			...
			//This section was modify for ajax wicaweb project requirements
			
			var params= opts.submitRedirect.split('/');

			if(params[0] == 'redirect_to_content')
			{
				if(params[1]){
					if(params[2]=='no'){
						$('#cms_container').load("/core/section_section/sectiondetails", {
							id : parseInt(params[1]),
							is_section_temp : parseInt(params[3])
						}, function() {
							// resize tree height according content
							setSectionTreeHeight();
							//scroll top
...
//		$('#single_img').append('<div id="id-'+file.uid+'" class="'+alt+'"><div class="agileUploaderFilePreview" style="display: none;"></div></div>');
